...
lv_obj_t * tile_stat_create(lv_obj_t * parent, const char * value_tile_stat, const char * unit, const char * label, lv_subject_t * value_tile_stat_2)
{
    ...
    if (lvpower_check_target(LVPOWER_TARGET_ALL)) {
        ...
        lv_obj_set_height(label_0, 31);
        lv_label_bind_text(label_0, value_tile_stat_2, NULL);
        lv_obj_set_style_text_color(label_0, CONST_TEXT_PRIMARY_GLOBAL, 0);
        lv_obj_add_style(label_0, &style_stat_md_global, 0);
        ...
        lv_label_set_translation_tag(label_2, label);
